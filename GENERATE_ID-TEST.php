...
function generateID() {
    ...
    // Build contact lines only if values exist to avoid empty gaps
    const contactValues = [emp_contact_person.value.trim(), emp_contact_address.value.trim(), emp_contact_number.value.trim()];
    const contactHTML = contactValues.filter(Boolean).map(s => `<p>${escape(s)}</p>`).join('') || '';
